...
_icv_open()
{
	utf8_state_t *cd = (utf8_state_t *)calloc(1, sizeof(utf8_state_t));

	if (cd == (utf8_state_t *)NULL) {
		errno = ENOMEM;
		return((void *)-1);
	}
...
size_t
_icv_iconv(utf8_state_t *cd, char **inbuf, size_t *inbufleft, char **outbuf,
                size_t *outbufleft)
{
	size_t ret_val = 0;
	uchar_t *ib;
